...
def FG_predCount(protocol, subj, run):

    test_subj = 'test'+subj

    folder = os.path.join('../',protocol,'/subject/',test_subj,'/segmentation/engy_run',str(run),'_pred_label_thre0.5')
    # disp(strcat(folder,'/seg_labels.csv'));
    labels = pd.read_csv(os.path.join(folder,'/seg_labels.csv'), names=['label'])
