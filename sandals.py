...
class window_sized(tkinter.Tk):
	def __init__(self, title="Window", window_width=100, window_height=100, **kw):
		tkinter.Tk.__init__(self)
		self.title(title)
		self.kw = kw
	...
	def __enter__(self):
		...
		# create canvas
		self.canvas = tkinter.Canvas(self,
						yscrollcommand=self.vscrollbar.set, bd=5, width=self.window_width, height=self.window_height)
		self.canvas.grid(row=0, column=0, sticky=N+S+E+W)
...
# class figure(tkinter.Label, object):
# 	def __init__(self, image="", **kw):
# 		self.kw  = kw
# 		self.imagevariable = tkinter.StringVar()
# 		self.imagevariable.set(self.kw['image'] if 'image' in self.kw else image)
# 		if 'image' in self.kw:
# 			del self.kw['image']
...
# 		tkinter.Label.__init__(self, _root, image = img, **kw)
# 		#The Label widget is a standard Tkinter widget used to display a text or image on the screen.
# 		self.pack(side=_pack_side)
		
# 	@property
# 	def image(self):
# 		return self.imagevariable.get()
	
# 	@image.setter
# 	def image(self, image):
# 		self.imagevariable.set(image)
...
class textWithVar(tkinter.Text, object):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.tk.eval('''
            proc widget_proxy {widget widget_command args} {
                ...
                # call the real tkinter widget command with the real args
